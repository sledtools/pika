...
- `crates/pikaci/src/ci_catalog.rs`: checked-in source of truth for canonical pre-merge lane selection and nightly lane definitions. Branch pushes are evaluated against the branch head's proposed manifest; nightly uses the default branch manifest.
...
- Branch-push pre-merge CI is now orchestrated by the forge from `crates/pikaci/src/ci_catalog.rs`.
