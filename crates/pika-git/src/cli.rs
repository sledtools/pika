...
pub struct LocalArgs {
    ...
    /// Output HTML file path. Defaults to `./pika-git-local.html`.
    #[arg(long)]
...
mod tests {
    ...
    fn serve_args_resolve_relative_paths_against_cwd() {
        let args = ServeArgs {
            config: PathBuf::from("pika-git.toml"),
            bind: None,
            port: None,
            db: PathBuf::from(".tmp/pika-git.db"),
            max_prs: 2,
        };
        let cwd = PathBuf::from("/tmp/pika-git-dev");
        assert_eq!(
            args.resolved_config_path(&cwd),
            PathBuf::from("/tmp/pika-git-dev/pika-git.toml")
        );
        assert_eq!(
            args.resolved_db_path(&cwd),
            PathBuf::from("/tmp/pika-git-dev/.tmp/pika-git.db")
        );
    }
    ...
    fn serve_args_keep_absolute_paths() {
        let args = ServeArgs {
            config: PathBuf::from("/srv/pika-git.toml"),
            bind: None,
            port: None,
            db: PathBuf::from("/srv/pika-git.db"),
            max_prs: 2,
        };
        let cwd = PathBuf::from("/tmp/pika-git-dev");
        assert_eq!(
            args.resolved_config_path(&cwd),
            PathBuf::from("/srv/pika-git.toml")
        );
        assert_eq!(
            args.resolved_db_path(&cwd),
            PathBuf::from("/srv/pika-git.db")
        );
    }
}
