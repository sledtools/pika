...
fn build_manifest(config: &ResolvedConfig, fixture: &RunningFixture) -> Manifest {
    ...
    Manifest {
        ...
        agent_api_token: fixture.server.as_ref().map(|s| s.agent_api_token.clone()),
...
fn print_summary(config: &ResolvedConfig, manifest: &Manifest) {
    eprintln!();
    eprintln!("=== pikahut ready ({}) ===", config.profile);
    eprintln!();
    ...
    if let Some(ref token) = manifest.agent_api_token {
