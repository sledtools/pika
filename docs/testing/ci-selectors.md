...
| `pre-merge-agent-contracts` | staged Linux agent-provider contract target: `pika-agent-control-plane` unit tests, `pika-agent-microvm` tests, `pika-server` `agent_api::tests`, and the `pika_core` NIP-98 signing contract test. It intentionally no longer owns the old host-side `pikahut` deterministic HTTP / CLI selectors; those are currently manual-only until they are rewritten against the surviving Incus/OpenClaw contract and assigned a new truthful lane. |
