...
| `just pre-merge-agent-contracts` | checked-in staged Linux agent-provider lane | none; this lane now owns the staged Rust contract target directly instead of host-side `pikahut` selectors | deterministic | pre-merge CI-owned: `pre-merge-agent-contracts` | none | Covers the surviving staged provider contract surface: `pika-agent-control-plane`, `pika-agent-microvm`, `pika-server` `agent_api::tests`, and the `pika_core` NIP-98 signing test. The old host-side deterministic HTTP / CLI selectors were intentionally removed because they still encode legacy vm-spawner assumptions; they are currently manual-only until they are rewritten against the surviving Incus/OpenClaw contract and assigned a new lane. |
...
| `pre-merge-agent-contracts` | staged Linux agent-provider contract target (`pika-agent-control-plane`, `pika-agent-microvm`, `pika-server` `agent_api::tests`, `pika_core` NIP-98) |
