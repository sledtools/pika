...
- `interop-rust-repo`: `pika-interop-lab-rust` checkout available.
...
| `just interop-rust-baseline` | `cargo test -p pikahut --test integration_deterministic interop_rust_baseline -- --ignored --nocapture` | `integration_deterministic::interop_rust_baseline` | heavy | manual-only | interop-rust-repo | Capability-gated skip when interop repo is missing; fails fast on workspace/harness MLS revision skew. Useful heavy contract, but not currently owned by a GitHub lane. |
...
| `just pre-merge-agent-contracts` | checked-in staged Linux agent-provider lane | none; this lane now owns the staged Rust contract target directly instead of host-side `pikahut` selectors | deterministic | pre-merge CI-owned: `pre-merge-agent-contracts` | none | Covers the surviving staged provider contract surface: `pika-cloud`, `pika-server` `agent_api::tests`, and the `pika_core` NIP-98 signing test. The old host-side deterministic HTTP / CLI selectors were intentionally removed because they still encode legacy pre-Incus assumptions; they are currently manual-only until they are rewritten against the surviving Incus/OpenClaw contract and assigned a new lane. |
