{
  ...
  outputs = { self, nixpkgs, crane, flake-utils, moq, rust-overlay, android-nixpkgs, disko, sops-nix, nix-openclaw }:
    let
      ...
      mkIncusGuestImagePkg = import ./nix/incus/guest-image-package.nix {
        inherit serverPkgs;
        nixpkgsLib = nixpkgs.lib;
      };

      incusGuestImageRoles = import ./nix/incus/roles.nix {
        inherit openclawGatewayPkg pikachatPkg;
      };

      mkIncusGuestRoleImagePkg = role:
        mkIncusGuestImagePkg {
          inherit (role) system description variant diskSize modules specialArgs;
          name = role.imageName;
          packageName = role.packageAttr;
          buildInstructions = "nix build .#packages.x86_64-linux.${role.packageAttr}";
          importInstructions =
            "./scripts/incus-role-image.sh --role ${role.id} build-import --remote-host pika-build --alias ${role.defaultAlias}";
        };

      managedOpenclawIncusImagePkg =
