...
if [[ -z "${STATE_DIR:-}" ]]; then
  AUTO_STATE_DIR=1
  STATE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/pikachat-openclaw.phase2.XXXXXX")"
fi
RELAY_URL="${RELAY_URL:-}"
...
cargo run --manifest-path "${REPO_ROOT}/Cargo.toml" -p pikachat -- scenario invite-and-chat-rust-bot --relay "${RELAY_URL}" --state-dir "${STATE_DIR}"
