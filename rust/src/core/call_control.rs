...
impl AppCore {
    ...
    fn current_peer_npub(&self, chat_id: &str) -> Option<String> {
        ...
        let g = sess.groups.get(chat_id)?;
        // For 1:1 DMs, members contains a single peer.
        // For note-to-self chats, members is empty; fall back to own pubkey.
        g.members
            .first()
            .and_then(|(pk, _, _)| pk.to_bech32().ok())
            .or_else(|| sess.keys.public_key().to_bech32().ok())
    }
