...
pub(crate) struct CallTrackSpec {
    pub name: String,
    pub codec: String,
    pub sample_rate: u32,
    pub channels: u8,
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub(crate) struct CallSessionParams {
    pub moq_url: String,
...
pub(crate) enum ParsedCallSignal {
    Invite {
        call_id: String,
...
pub(crate) enum OutgoingCallSignal<'a> {
...
#[derive(Debug, Clone)]
pub(crate) struct CallMediaCryptoContext {
...
pub(crate) fn valid_relay_auth_token(token: &str) -> bool {
...
pub(crate) fn parse_call_signal(content: &str) -> Option<ParsedCallSignal> {
...
pub(crate) fn build_call_signal_json(
    call_id: &str,
...
pub(crate) fn derive_call_media_crypto_context(
    ctx: &CallCryptoDeriveContext<'_>,
    primary_track: &str,
...
pub(crate) fn derive_relay_auth_token(ctx: &CallCryptoDeriveContext<'_>) -> Result<String, String> {
    let shared_seed = call_shared_seed(
        ctx.call_id,
        ctx.session,
        ctx.local_pubkey_hex,
...
pub(crate) fn validate_relay_auth_token(ctx: &CallCryptoDeriveContext<'_>) -> Result<(), String> {
