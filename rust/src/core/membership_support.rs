...
pub(crate) fn prepare_add_members(
    ...
    MembershipRuntime::new(mls).prepare_add_members(mls_group_id, key_package_events)
}
...
pub(crate) fn prepare_remove_members(
    ...
    MembershipRuntime::new(mls).prepare_remove_members(mls_group_id, removed_pubkeys)
}
...
pub(crate) fn prepare_leave_group(
    ...
    MembershipRuntime::new(mls).prepare_leave_group(mls_group_id)
}
...
pub(crate) fn prepare_group_data_update(
    ...
    MembershipRuntime::new(mls).prepare_group_data_update(mls_group_id, update)
}
...
pub(crate) fn validate_key_package_events(
    ...
    shared_validate_key_package_events(mls, key_package_events)
}

pub(crate) fn clear_pending_commit(mls: &PikaMls, mls_group_id: &GroupId) -> Result<()> {
    shared_clear_pending_commit(mls, mls_group_id)
}
...
pub(crate) fn finalize_published_evolution(
    ...
    MembershipRuntime::new(mls).finalize_published_evolution(prepared)
}
