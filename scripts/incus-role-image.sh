...
usage() {
  cat <<'EOF'
Usage:
  scripts/incus-role-image.sh --role ROLE build
  scripts/incus-role-image.sh --role ROLE import [--remote-host HOST] [--project PROJECT] [--alias ALIAS] [--artifact PATH]
  scripts/incus-role-image.sh --role ROLE build-import [--remote-host HOST] [--project PROJECT] [--alias ALIAS]
...
build_image() {
  nix build ".#packages.x86_64-linux.${package_attr}"
}
...
build_and_import_on_remote() {
  ...
  ssh_remote "
    set -euo pipefail
    cd '$remote_tmp'
    nix build .#packages.x86_64-linux.${package_attr} --out-link result
    incus_cmd='incus'
    if [[ \$(id -u) -ne 0 ]]; then
      incus_cmd='sudo incus'
    fi
