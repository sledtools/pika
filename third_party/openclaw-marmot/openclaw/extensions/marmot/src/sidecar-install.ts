...
const DEFAULT_REPO = "justinmoon/openclaw-marmot";
...
async function fetchRelease(params: {
  ...
  const normalized: GitHubReleaseAsset[] = assets
    .map((a: any) => ({
  ...
  if (!tagName || normalized.length === 0) {
    throw new Error("release payload missing tag_name/assets");
  }
  return { tag_name: tagName, assets: normalized };
}
