...
- Next seam:
  remove the remaining `pika-marmot-runtime` facade slice from `pika_core`: the call/call-runtime workflow helpers that still leak through `host_context`, `call_control`, and `call_runtime`.
