...
- Next seam:
  remove the remaining runtime relay/session helper residue from `pika_core`, starting with `RuntimeRelayRolePlan` in `session.rs` / `config.rs` and the leftover notification/session helper calls in the session path.
