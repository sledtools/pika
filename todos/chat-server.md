...
- Next seam:
  remove the next `pika-marmot-runtime` facade slice from `pika_core`, starting with the conversation storage/query wrappers (`RuntimeJoinedGroupSnapshot`, `RuntimeMessagePageQuery`, `RuntimeMessagePage`, and the remaining `ConversationRuntime` accessors).
