...
- Next seam:
  remove the next `pika-marmot-runtime` facade slice from `pika_core`, starting with outbound publish + membership helper calls in `host_context` and the remaining runtime-owned result/status types they still expose.
