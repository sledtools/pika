...
- Next seam:
  remove the next `pika-marmot-runtime` facade slice from `pika_core`, starting with the small relay publish wrapper and then the remaining call/call-runtime workflow helpers that still leak through `host_context`, `call_control`, and `call_runtime`.
