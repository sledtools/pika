...
- Next seam:
  remove the next `pika-marmot-runtime` facade slice from `pika_core`, starting with welcome staging/delivery helpers (`PendingWelcomeSnapshot`, welcome lookup/listing, and publish pairing) while deferring conversation ingress/decrypt interpretation for a later chunk.
