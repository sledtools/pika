...
- [x] Localize the media helper/type slice:
  `pika_core` now owns encrypted media upload/download helpers plus the upload/result blob types in `core/media_support.rs`, and app code no longer imports `pika_marmot_runtime::media::*`.
...
- [x] Localize the conversation ingress/decrypt slice:
  `pika_core` now owns group-message processing, processing-result interpretation, and relay backlog ingest in `core/conversation_support.rs`, so app chat flows no longer import `pika_marmot_runtime::conversation::*`.
- [x] Localize the outbound publish + membership helper slice:
  `pika_core` now owns outbound action preparation/publish status plus membership evolution preparation/finalization in `core/outbound_support.rs` and `core/membership_support.rs`, so app code no longer imports `pika_marmot_runtime::outbound::*` or `pika_marmot_runtime::membership::*`.
- [x] Localize message classification and key-package interop helpers:
  `pika_core` now owns message kind constants/classification in `core/message_support.rs` plus key-package normalization/relay extraction in `core/interop.rs`, so app code no longer imports `pika_marmot_runtime::message::*` or `pika_marmot_runtime::key_package::*`.
- [x] Localize the relay publish retry helper:
  `pika_core` now owns `PublishOutcome` plus relay publish retry/backoff in `core/relay_publish.rs`, so app code no longer imports `pika_marmot_runtime::relay::*`.
- [x] Localize the call signal/workflow helper slice:
  `pika_core` now owns call signal parsing/building, relay-auth/media-crypto derivation, and inbound/outbound call workflow preparation in `core/call_support.rs` and `core/call_workflow.rs`, so `rust/src/core` no longer imports `pika_marmot_runtime::*`.
...
- [x] Route app and sidecar MLS imports through one local seam:
...
- Next seam:
  finish collapsing the now-internal `RuntimeCommands` / `RuntimeQueries` shim inside `pikachat-sidecar`, then keep shrinking the raw `PikaMdk` surface from inside `pika-mls`, starting with the remaining message/welcome escape hatches and the direct wrapper consumers in CLI/NSE.
...
- The first two post-runtime MDK cuts are now in place.
  `pika-mls` owns the MLS storage bootstrap and common engine reexports, CLI/NSE/app/sidecar now route their MLS imports through that crate, and direct `mdk-*` manifest deps are down to the workspace root, CI workspace, and `pika-mls` itself.
- The MDK seam is now explicit in code too.
  `PikaMdk` is a concrete local wrapper, which means future replacement work can happen inside `crates/pika-mls` without another repo-wide type churn first.
- The remaining MDK replacement work is now much more honest.
...
- The call-media escape hatch is gone from production code.
  The only remaining `as_raw()` uses in-tree are unrelated raw buffer / JNI handles, not MDK engine reach-through from app or sidecar call flows.
