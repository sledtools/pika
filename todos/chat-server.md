...
- Next seam:
  remove the next `pika-marmot-runtime` facade slice from `pika_core`, starting with message/call workflow helpers and key-package interop that still leak through `host_context`, `call_control`, and `interop`.
