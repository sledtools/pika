...
- third Phase 4 slice landed: shared `RuntimeCommands` now also exposes membership evolution preparation/finalization for the already-shared add-members/evolution workflow
...
- app call signal publishing now routes invite/accept/reject/end failure handling through that operation result family, and daemon `InviteCall` / `AcceptCall` / `RejectCall` / `EndCall` publish paths now consume the same shared result boundary while media runtime ownership, protocol mapping, and broader call state orchestration remain host-local
...
- single-item media send/upload completion in app and daemon now unwraps shared media upload operation results via runtime-owned helpers instead of host-local `match` trees, and the app media workflow now keeps shared `PreparedMediaUpload` through pending single/batch state, stores shared `RuntimeMediaUploadResult` on completed batch items, and resumes retry/finalize flow directly from those shared runtime types instead of splitting uploads back into app-local `upload`/`encrypted_data`/URL/hash tuples
...
- app call-control now routes start/accept/reject/end signal preparation directly through the shared runtime call command boundary, reuses one app-local dispatch path for best-effort reject/end follow-through, and unwraps call signal publish failures through the shared call signal publish operation result family alongside the daemon’s real `InviteCall` / `AcceptCall` / `RejectCall` / `EndCall` publish flows; actual call worker/media ownership, protocol mapping, UI state transitions, and retry/orchestration stay host-local
